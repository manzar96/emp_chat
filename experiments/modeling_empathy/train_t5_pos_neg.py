...
from core.data.empdataset import EmpatheticDataset, EmpatheticDatasetPosNeg
from core.data.persona import PersonaChatDataset
from core.data.collators import T5CollatorEmpChatEmo, \
    T5CollatorEmpChatEmoPosNegSampling, T5CollatorPersChat
from core.models.huggingface.t5_extended import  \
    T5ConditionalGenerationEmotionsSharedPosNeg

from core.trainers import T5TransformerTrainerPosNeg
...
if options.dataset_name == "empchat":
    train_dataset = EmpatheticDatasetPosNeg("train", options.max_hist_len)
...
# load data
if options.dataset_name == "empchat":
    collator_fn1 = T5CollatorEmpChatEmoPosNegSampling(device='cpu')
elif "persona":
    collator_fn1 = T5CollatorPersChat(device='cpu')

if options.dataset_name == "empchat":
    collator_fn2 = T5CollatorEmpChatEmo(device='cpu')
elif "persona":
    collator_fn2 = T5CollatorPersChat(device='cpu')
...
model = T5ConditionalGenerationEmotionsSharedPosNeg(lm_model=lm_model,
                                          num_classes=2,
                                          device=DEVICE)
...
# create trainer
trainer = T5TransformerTrainerPosNeg(model=model,
                                  optimizer=optimizer,
                                  patience=5,
                                  criterion=criterion,
                                  scheduler=None,
